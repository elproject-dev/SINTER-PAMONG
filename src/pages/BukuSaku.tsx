...
import { Search, Plus, Trash2, FileText, Loader2, Users, ChevronDown, Check, Paperclip, User as UserIcon, Edit, Download, SlidersHorizontal, ArrowDown, ArrowUp, X } from 'lucide-react';
  ...
  return (
    <div className="w-full space-y-8 animate-in fade-in duration-300">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-slate-800 dark:text-slate-50 mb-1 sm:mb-2 tracking-tight">Buku Saku</h1>
          <p className="text-slate-500 dark:text-slate-400 text-sm sm:text-base lg:text-lg">Kumpulan referensi, panduan, dan data media tugas</p>
        </div>
      ...
      {showForm && user.role === 'admin' && (
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6 animate-in slide-in-from-top-4 fade-in duration-300">
          <h2 className="font-bold text-slate-800 dark:text-slate-50 text-lg mb-4 flex items-center gap-2 border-b border-slate-100 dark:border-slate-700 pb-3">
            <BiBookBookmark size={20} className="text-school-blue dark:text-white" />
            {editingMediaId ? 'Edit Media Buku Saku' : 'Upload Media Baru'}
          </h2>
          <form onSubmit={handleUploadSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1">
                <label className="text-sm font-bold text-slate-700 dark:text-slate-200">Judul Media <span className="text-rose-500">*</span></label>
                <input
                  type="text"
                  value={judul}
              ...
              <div className="space-y-1">
                <label className="text-sm font-bold text-slate-700 dark:text-slate-200">File Upload {editingMediaId ? <span className="text-slate-400 font-medium">(Opsional)</span> : <span className="text-rose-500">*</span>}</label>
                <input
                  type="file"
                  ref={fileInputRef}
                    ...
                    <div className="absolute z-20 w-full mt-2 bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-xl shadow-xl overflow-hidden animate-in fade-in slide-in-from-top-2">
                      ...
                      <div className="max-h-[200px] overflow-y-auto py-1 custom-scrollbar">
                        ...
                        >
                          ...
                          <span className="text-xs text-slate-500 dark:text-slate-400">Media informasi untuk semua pengguna</span>
                        </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
        <div className="p-4 border-b border-slate-200 dark:border-slate-700 flex flex-col sm:flex-row items-center justify-between bg-white dark:bg-slate-800 gap-3 relative z-10 rounded-t-xl">
          <div className="flex items-center space-x-2 truncate w-full sm:w-auto">
            <BiBookBookmark size={20} className="text-slate-600 dark:text-slate-300 shrink-0" />
            <h2 className="font-bold text-slate-800 dark:text-slate-50 text-lg truncate">Daftar Media ({filteredMedia.length})</h2>
          </div>
          ...
          <table className="w-full text-left border-collapse min-w-[900px] hidden xl:table">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-900 text-slate-500 dark:text-slate-400 text-xs uppercase tracking-wider">
                ...
                <th className="px-4 py-3 font-bold border border-slate-200 dark:border-slate-700">JUDUL MEDIA</th>
              ...
              {isLoading ? (
                    ...
                    <td className="px-4 py-3 border border-slate-200 dark:border-slate-700 text-sm text-slate-800 dark:text-slate-50">
                      {media.targetUserId ? (
                        ...
                        <>
                          ...
                          <div className="text-xs text-slate-500 dark:text-slate-400 font-medium">Media informasi untuk semua pengguna</div>
                        </>
                      )}
                    </td>
                ...
                <tr>
                  <td colSpan={6} className="p-12 text-center text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700">
                    ...
                    <p className="font-bold text-lg text-slate-600 dark:text-slate-300 mb-1">Tidak Ada Media</p>
